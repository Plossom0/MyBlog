...
export function fetchPosts(
  ...
  return fetch(url, { headers: authHeader() }).then((r) => handle<PostMeta[]>(r, '获取文章列表失败'))
}

export function fetchPost(id: number): Promise<PostDetail> {
  return fetch(`${BASE}/posts/${id}`, { headers: authHeader() }).then((r) =>
    handle<PostDetail>(r, '获取文章失败'),
  )
}

export function fetchTags(): Promise<TagWithCount[]> {
  return fetch(`${BASE}/tags`, { headers: authHeader() }).then((r) => handle<TagWithCount[]>(r, '获取标签失败'))
}

export function fetchCategories(): Promise<CategoryWithCount[]> {
  return fetch(`${BASE}/categories`, { headers: authHeader() }).then((r) =>
    handle<CategoryWithCount[]>(r, '获取分类失败'),
  )
}

export function searchPosts(q: string): Promise<PostSearchResult[]> {
  return fetch(`${BASE}/search?q=${encodeURIComponent(q)}`, { headers: authHeader() }).then((r) =>
...
export function createPost(payload: CreatePostPayload): Promise<{ id: number }> {
  ...
  if (payload.public !== undefined) form.append('public', String(payload.public))
  return fetch(`${BASE}/posts`, {
    method: 'POST',
    headers: authHeader(),
    body: form,
  }).then((r) => handle<{ id: number }>(r, '上传失败'))
}
...
export function deletePost(id: number): Promise<{ success: true }> {
  return fetch(`${BASE}/posts/${id}`, {
    method: 'DELETE',
    headers: authHeader(),
  }).then((r) => handle<{ success: true }>(r, '删除失败'))
}
...
export function uploadImage(file: File): Promise<{ url: string }> {
  ...
  return fetch(`${BASE}/upload/image`, {
    ...
    headers: authHeader(),
    body: form,
  }).then((r) => handle<{ url: string }>(r, '图片上传失败'))
}
